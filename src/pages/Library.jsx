...
export default function Library() {
  ...
  const filteredJourneys = useMemo(() => {
    let list = filter !== "all" ? journeys.filter(j => j.track === filter) : journeys;
    if (query) {
      const q = query.toLowerCase();
      list = list.filter(j => j.title.toLowerCase().includes(q) || (j.summary ?? "").toLowerCase().includes(q) || (j.tags ?? []).some(t => t.toLowerCase().includes(q)));
    }
    return list;
  }, [journeys, filter, query]);

  const filteredBoosts = useMemo(() => {
    let list = filter !== "all" ? boosts.filter(b => b.category === filter) : boosts;
    if (query) {
      const q = query.toLowerCase();
      list = list.filter(b => b.title.toLowerCase().includes(q) || (b.subtitle ?? "").toLowerCase().includes(q) || (b.tags ?? []).some(t => t.toLowerCase().includes(q)));
    }
    return list;
  }, [boosts, filter, query]);
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 md:px-8 py-6 md:py-10 space-y-6">
      ...
      <div className="flex border-b border-border">
        {[{ id: "journeys", label: "Journeys", count: journeys.length }, { id: "boosts", label: "Boosts", count: boosts.length }].map(t => (
      ...
      <div className="flex gap-2 overflow-x-auto no-scrollbar -mx-4 px-4 md:mx-0 md:px-0 pb-1">
        {filterOptions.map(opt => (
          <button key={opt} onClick={() => setFilter(opt)}
            className={cn("shrink-0 text-xs px-3 py-1.5 rounded-full border transition-colors capitalize",
              filter === opt
      ...
      {loading ? (
        <div className="flex justify-center py-16"><div className="w-5 h-5 rounded-full border-2 border-border border-t-accent animate-spin" /></div>
      ) : tab === "journeys" ? (
        filteredJourneys.length === 0 ? <EmptyState label="No journeys match your filters." /> : (
          <div className="space-y-2">
            {filteredJourneys.map(j => <JourneyCard key={j.id} journey={j} navigate={navigate} />)}
          </div>
        )
      ) : (
        filteredBoosts.length === 0 ? <EmptyState label="No boosts match your filters." /> : (
          <div className="space-y-2">
            {filteredBoosts.map(b => <BoostCard key={b.id} boost={b} done={completedSet.boost.has(b.id)} navigate={navigate} />)}
          </div>
        )
      )}
    </div>
...
function JourneyCard({ journey, navigate }) {
  return (
    <div onClick={() => navigate(`/journey/${journey.slug}`)}
...
function BoostCard({ boost, done, navigate }) {
  return (
    <div onClick={() => navigate(`/boost/${boost.id}`)}
      className="cursor-pointer rounded-lg border border-border bg-surface-1 p-4 hover:border-border-strong transition-colors"
    >
      <div className="flex items-start gap-3">
