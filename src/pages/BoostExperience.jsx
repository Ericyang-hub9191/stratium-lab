      ...
      try {
        ...
        if (currentUser && rec) {
          const existing = await base44.entities.UserProgress.filter({
            userId: currentUser.id, contentId: rec.id, contentType: "boost",
          });
          if (existing[0]) {
            ...
            const created = await base44.entities.UserProgress.create({
              userId: currentUser.id, contentId: rec.id, contentType: "boost",
              status: "in-progress", checkAnswers: {}, practiceEntries: {}, writeEntries: {},
              startedAt: new Date().toISOString(), xpAwarded: 0,
            });
            if (!cancelled) setProgress(created);
          }
        }
      } catch (e) { console.error("Failed to load boost:", e); }
      ...
      <div className="flex-1 reading-surface py-8 md:py-12 px-4 md:px-8 animate-fade-in">
        <article className="max-w-2xl mx-auto space-y-6 pb-28">
          <header className="space-y-3 pb-4 mb-2 border-b" style={{ borderColor: "hsl(var(--reading-border))" }}>
            ...
            {boost.subtitle && <p className="text-[1.1em] muted !mt-1" style={{ lineHeight: 1.5 }}>{boost.subtitle}</p>}
          </header>
        ...
        <div className="max-w-2xl mx-auto px-4 md:px-6 py-3.5 flex items-center gap-4">
          <div className="flex-1 text-xs text-text-secondary">
            {isCompleted
              ...
              : requiredChecks.length > 0 && !canComplete ? <span>Answer the checkpoint to finish.</span>
              : canComplete ? <span className="text-text-primary">Ready.</span>
